...
if [ "$branch_name" == "$ticket_number" ]; then
	echo "$TEXT_ERROR[ >>> COMMIT REJECTED - Ticket number missing in commit message & branch name ]\n"\
		"Malformed branch name. Branches should be named:\n"\
		"  * ticketType/TICKET-5-some-branch-name\n"\
		"  * TICKET-5-some-branch-name\n"\
		"If you absolutely need to commit this use git commit --no-verify\n"\
		"[ <<< COMMIT REJECTED - Ticket number missing in commit message & branch name ]"
